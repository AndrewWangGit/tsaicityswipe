...
const App = () => {
  ...
  // the admin emergency note (to close TSAI CITY)
  const [emergencyNote, setEmergencyNote] = useState("");

  // whether or not to close TSAI CITY, dependent on several factors
  const [isOpen, setIsOpen] = useState(false);

  const [timer, setTimer] = useState(null);

  // check whether TSAI CITY should be open everytime the database changes
  useEffect(() => {
    checkIsOpen();
  }, [userTable, capacityCount, timer, emergencyNote]);

  // check whether TSAI CITY should be open every minute
  useEffect(() => {
  ...
  const checkIsOpen = () => {
    // if there is a force close in the firebase, close TSAI CITY
    // and don't even check the other conditions
    let checkOne = !emergencyNote;

    console.log("check one", checkOne);

    // if there are too many people, close TSAI CITY,
    // and don't even check the other conditions
